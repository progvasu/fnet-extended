...
def train(loader, model, optimizer, exp_logger, epoch, train_all, print_freq=100, clip_gradient=True, iter_size=1):
    """
        ...
        Inputs:
            ...
            optimizer: optimizer
            exp_logger: logger okay! exp?
            epoch: current epoch number
            train_all: ?
            print_freq: ?
            clip_gradient: preventing gradient overflow
            iter_size: ?
    """
    model.train()
    ...
    for i, sample in enumerate(loader): # (im_data, im_info, gt_objects, gt_relationships)
        ...
        input_visual = [item for item in sample['visual']] # extract image features
        target_objects = sample['objects']
        target_relations = sample['relations']
        image_info = sample['image_info'] # image rescaled and original sizes and scale
        ...
        try:
            ...
            loss.backward() # propagate the loss backwards but don't update
        ...
        if  (i + 1) % print_freq == 0:
            ...
            print('\tloss_cls_obj: {loss_cls_obj.avg:.4f} '
